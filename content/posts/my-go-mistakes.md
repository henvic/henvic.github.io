...
**Where I regret using it?** I used `init` functions to define command structures and flags when using [cobra](https://github.com/spf13/cobra) to create CLI tools, but now I see it was unnecessary.
By the way, Go 1.16 was released yesterday and has good news.
...
Once I read that you should only write comments when you fail to express yourself with code because they might get outdated and out of sync with code.
...
I used to underestimates comments subconsciously following this mindset.
Thankfully, after a couple of years working with Go, I learned to value the power of good comments.
If you have doubts about what is reasonable or not to express with comments, I suggest reading the standard library's source code and seeing how they do.
...
It's relatively straight-forward to use it, and you gain consistency in following it even if it's not your preferred style!

**Remember:** Code is written once but read multiple times. It's better to spend a little more time writing comments to your code in a way that others and your future self will understand than to hurry and spend ten times more trying to make sense of it months or years later.
...
[cmd/list package](https://github.com/henvic/wedeploycli/tree/master/command/list) is one of the worst offenders:
...
Use internal packages to protect all your code but what needs a public API.
...
If you're worried about this idea because for sure your code might be useful throughout your team projects, it might be a good time to reflect on the following [Go proverb](https://go-proverbs.github.io/):
...
See [example](https://play.golang.org/p/paDWRN_K5LZ) in the Go Playground for a working example.
...
This build process can easily create take as input a couple of text files containing only a few Kilobytes and generate a binary file of Megabytes after converting your code to machine-friendly format and linking dependencies.
...
It almost doubled the compressed file size from slightly over 4 to 9 Megabytes.
...
I didn't learn about concurrency with Go, but I committed the most rookie possible quite a few times: Writing from multiple goroutines or Threads will cause text to be all mingled.

I made this error even more so when writing packages for text animation in the terminal.
If you find yourself with this issue, you can probably use a mutex to set what goroutine can write at the moment, or in more complicated cases, use channels to print from a single place.
...
Something fun I solved was when implementing “SSH over WebSocket” feature.
...
It required arduous work understanding how the protocol worked to make it possible to do what we needed.
...
**You usually want to try to postpone the creation of goroutines until the latest moment.**
